...
public class AnalyticsServiceImpl implements AnalyticsService {
    ...
    public Contribution getContributions(String username, String mode) {
        return repository
                .findByUser_GithubUsernameAndMode(username, mode)
                .orElseGet(
                        () -> {
                            try {
                                Contribution newContribution =
                                        getContributionCalendar(username, mode);
    ...
    public Contribution getContributionCalendar(String username, String mode) {
        ...
        Map<String, Object> response =
                gitHubClient.executeQuery(queryManager.getContributionCalendar(), variables);

        Contribution calendar = new Contribution();
        SuggestedUser githubUser =
                userRepository
        ...
        if (response != null && response.containsKey("user")) {
            ...
            Map<String, Object> contributionCalendarData =
                    (Map<String, Object>) contributionsCollection.get("contributionCalendar");

            int pull_requests =
                    (Integer) contributionsCollection.get("totalPullRequestContributions");
            ...
            List<Map<String, Object>> weeksData =
                    (List<Map<String, Object>>) contributionCalendarData.get("weeks");
            if (mode.equals("weekly")) {
                ...
                for (Map<String, Object> weekData : weeksData) {
                    ...
                    List<Map<String, Object>> daysData =
                            (List<Map<String, Object>>) weekData.get("contributionDays");
                    int weekCommits = 0;
