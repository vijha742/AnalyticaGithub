...
public class SuggestedUserServiceImpl implements SuggestedUserService {
	...
	public SuggestedUser suggestUser(String githubUsername, String team) {
        ...
        User suggestedBy = userRepository.getReferenceById(authenticatedUser.getId());
        suggestedUser.setSuggestedBy(suggestedBy);
        // Debug logs
        log.info("Auth user class: {}", authenticatedUser.getClass().getName());
        log.info("Auth user ID: {}", authenticatedUser.getId());
        log.info("Auth user username: {}", authenticatedUser.getGithubUsername());
        log.info("Fetched user ID: {}", suggestedBy.getId());
        ...
        suggestedUser.setPullRequestsCount(githubUser.getContributionsCollection() != null ? githubUser.getContributionsCollection().getTotalPullRequestContributions() : 0);
        suggestedUser.setIssuesCount(githubUser.getContributionsCollection() != null ? githubUser.getContributionsCollection().getTotalIssueContributions() : 0);
        suggestedUser.setCommitsCount(githubUser.getContributionsCollection() != null ? githubUser.getContributionsCollection().getTotalCommitsCount() : 0);
        suggestedUser.setTotalContributions(suggestedUser.getCommitsCount() + suggestedUser.getIssuesCount() + suggestedUser.getPullRequestsCount());
        ...
        if (githubUser.getRepositories() != null && githubUser.getRepositories().getNodes() != null) {
            for (GitHubUserResponse.Repository repo : githubUser.getRepositories().getNodes()) {
                ...
                log.info("Mapped repository: {}", mappedRepo.getName());
        ...
        log.info("Suggested user: {} with team: {}", githubUsername, team);
	...
	private Optional<GitHubUserResponse.User> addUserData(String githubUsername) {
        ...
        private SuggestedGithubRepository mapRepository(GitHubUserResponse.Repository repo, SuggestedUser user) {
...
//
	...
	public List<SuggestedUser> getAllActiveUsers(String team) {
		return suggestedUserRepository.findByActiveTrueAndSuggestedByAndTeam((User) SecurityContextHolder.getContext().getAuthentication().getPrincipal(),team);
	}
//
//	@Override
...
//
//  			CompletableFuture<List<User>> future = CompletableFuture.supplyAsync(() -> fetchBatch(suggestedUsers, batchStart, batchEnd))
...
//
//		try {
//			Map<String, Object> response = githubClient.executeQuery(queryBuilder.toString(),
