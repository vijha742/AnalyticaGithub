...
//		Map<String, Object> response = gitHubClient.executeQuery(queryManager.analyzeReadmeQuality(),
//				variables);
...
//					if (target != null) {
//						Map<String, Object> history = (Map<String, Object>) target
//								.get("history");
//						if (history != null) {
//							List<Map<String, Object>> historyNodes = (List<Map<String, Object>>) history
//									.get("nodes");
//							if (historyNodes != null && !historyNodes.isEmpty()) {
//								String lastCommitDate = (String) historyNodes.get(0)
//										.get("committedDate");
...
//		if (hasIntro)
//			score += 25;
//		boolean hasInstall = Pattern
//				.compile("(?i)(installation|getting started|setup|how to use)", Pattern.MULTILINE)
//				.matcher(content).find();
//		quality.setHasInstallationGuide(hasInstall);
//		if (hasInstall)
//			score += 25;
//		boolean hasExamples = Pattern
//				.compile("(?i)(usage|example|examples|how to|usage example)", Pattern.MULTILINE)
//				.matcher(content).find();
//		quality.setHasUsageExamples(hasExamples);
//		if (hasExamples)
//			score += 25;
//		boolean hasMaintainer = Pattern
//				.compile("(?i)(contributing|maintainer|contact|support|license)", Pattern.MULTILINE)
//				.matcher(content).find();
...
//
//				if (languages != null) {
//					List<Map<String, Object>> edges = (List<Map<String, Object>>) languages
//							.get("edges");
...
//				if (totalFiles > 100)
//					factors.add("Large number of files");
//				if (languageStats.size() > 3)
//					factors.add("Multiple languages");
//				if (totalSize > 100000)
//					factors.add("Large codebase");
//				if (factors.isEmpty())
//					factors.add("Standard complexity");
...
//
	...
	public TechnicalProfile getTechnicalProfile(String username) {
		...
		if (response == null)
			return profile;
		Map<String, Object> user = (Map<String, Object>) response.get("user");
		if (user == null || !user.containsKey("repositories"))
			return profile;
		...
		for (Map<String, Object> repo : nodes) {
			...
			for (Map<String, Object> nodeSet : nodesList) {
				...
				if (isFramework(topic)) {
					frameworkMap.computeIfAbsent(topic,
					...
					frameworkMap.computeIfAbsent(topic,
							t -> new TechnologyUsage(topic, TechnologyCategory.TOOL))
							.update(createdAt, updatedAt);
				}
			}
			Map<String, Object> languages = (Map<String, Object>) repo.get("languages");
					...
					if (langNode != null) {
						...
						if (languageName != null) {
							languageMap.computeIfAbsent(languageName,
									k -> new LanguageExpertise(languageName))
									.update(createdAt, updatedAt, LinesCalculator
		...
		// TODO: To populate frameworkMap: - topics based, - dependency parsing for
		// limited projects, - leave at all (Currently topic based)
