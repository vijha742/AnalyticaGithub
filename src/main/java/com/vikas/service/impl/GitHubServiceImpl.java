...
public class GitHubServiceImpl implements GitHubService {
	...
	public User getUser(String Username) {
		User user = new User();
		try {
			Map<String, Object> response = githubClient.executeQuery(queryHub.fetchUserData(),
			...
			if (response != null && response.get("user") instanceof Map) {
				...
				if (gitHubUser.get("repositories") instanceof Map) {
					...
					if (repositories.get("nodes") instanceof List) {
						...
						for (Map<String, Object> map : repoNodes) {
							...
							repo.setStargazerCount(((Integer) map.get("stargazerCount")).intValue());
				...
				if (gitHubUser.get("contributionsCollection") instanceof Map) {
					Map<String, Object> contributionsCollection = (Map<String, Object>) gitHubUser.get("contributionsCollection");
					if (contributionsCollection != null && contributionsCollection.get("contributionCalendar") instanceof Map) {
						Map<String, Object> map = (Map<String, Object>) contributionsCollection.get("contributionCalendar");
